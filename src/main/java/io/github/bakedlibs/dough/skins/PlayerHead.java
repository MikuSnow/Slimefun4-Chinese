...
public final class PlayerHead {
     ...
     *
     * @param customName
     *              The custom Name used for skull owner
     * 
     * @return A new Head Item for the specified Player
     */
    public static @Nonnull ItemStack getItemStack(@Nonnull PlayerSkin skin, @Nonnull String customName) {
        Validate.notNull(skin, "The skin can not be null!");
        return getItemStack(customName, meta -> {
            try {
                CustomGameProfile.PLAYER_NAME = customName;
                skin.getProfile().apply(meta, customName);
                CustomGameProfile.PLAYER_NAME = "CS-CoreLib";
    ...
    private static @Nonnull ItemStack getItemStack(String customName, @Nonnull Consumer<SkullMeta> consumer) {
        ItemStack item = new ItemStack(Material.PLAYER_HEAD);
        SkullMeta meta = (SkullMeta) item.getItemMeta();
        consumer.accept(meta);
        item.setItemMeta(meta);
